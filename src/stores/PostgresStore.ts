...
export default class PostgresStore implements Store {
  ...
  async initSessionsTable() {
    ...
    } (id varchar(21) not null primary key, data varchar)`;
  }
  ...
  async createSession(sessionId: string, initialData: SessionData) {
    await this.sql`insert into ${
      this.sql(this.tableName)
    } (id, data) values (${sessionId}, ${JSON.stringify(initialData)})`;
  }
  ...
  async persistSessionData(sessionId: string, sessionData: SessionData) {
    await this.sql`update ${this.sql(this.tableName)} set data = ${
